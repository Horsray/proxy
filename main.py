...
def comfy_ws_listener():
    ...
    ws_url = sanitize_url(proxy.config.get("comfyui_url") or COMFYUI_URL)
    if not ws_url:
        logger.info("⏭️ 未配置 ComfyUI 地址，跳过 WebSocket 监听")
        return
...
class HuiYingProxy:
    ...
    def load_config(self, config_file):
        ...
        if default_config['comfyui_url']:
            logger.info(f"🔗 当前 ComfyUI 地址: {default_config['comfyui_url']}")
        else:
            logger.info("🔗 当前 ComfyUI 地址: 未配置")
...
#     try:
#         comfyui_url = request.args.get('comfyuiUrl') or COMFYUI_URL
#         comfyui_url = sanitize_url(comfyui_url)
...
#         }
#         comfyui_url = data.get('comfyuiUrl') or COMFYUI_URL
#         comfyui_url = sanitize_url(comfyui_url)
...
if __name__ == '__main__':
    ...
    if COMFYUI_URL:
        ...
        logger.info("⏭️ 未设置 ComfyUI 地址，跳过监听")
